<template>
  <form class="mx-auto flex  flex-col w-full max-w-xs  gap-6 py-10" @submit="onSubmit">
    <!-- Commission -->
    <fieldset class="space-y-2">
      ...
      <div class="inline-flex w-full rounded-md border border-slate-200 bg-slate-100 p-1">
        <input id="commission-yes" class="peer/yes sr-only" type="radio" name="commission" v :value="true"
          v-model="commission" />
        <label for="commission-yes" class="flex-1 cursor-pointer select-none rounded-md px-4 py-2 text-center text-sm font-semibold
                 text-slate-500 transition hover:text-slate-700
                 peer-checked/yes:bg-white peer-checked/yes:text-primary peer-checked/yes:shadow-sm
                 peer-checked/yes:ring-1 peer-checked/yes:ring-slate-200">
          Yes
        </label>

        <input id="commission-no" class="peer/no sr-only" type="radio" name="commission" :value="false"
          v-model="commission" />
        <label for="commission-no" class="flex-1 cursor-pointer select-none rounded-md px-4 py-2 text-center text-sm font-semibold
                 text-slate-500 transition hover:text-slate-700
                 peer-checked/no:bg-white peer-checked/no:text-primary peer-checked/no:shadow-sm
                 peer-checked/no:ring-1 peer-checked/no:ring-slate-200">
          No
        </label>
      </div>
    ...
    <!-- Property price -->
    <div class="space-y-1">
      <NumberStepper label="Property Purchase Price" :model-value="propertyPrice" :step="1000" :min="0" required
    ...
    <!-- Total savings -->
    <div class="space-y-1">
      <NumberStepper label="Total Savings" :model-value="totalSavings" :step="1000" :min="0" required
    ...
    <fieldset class="space-y-2">
      ...
      <div class="flex items-center w-full max-w-xs rounded-md border border-slate-200 bg-slate-100 px-3 py-2">
        <input id="annualRepaymentRate" name="annualRepaymentRate" inputmode="decimal" placeholder="0" class="min-w-0 flex-1 bg-transparent text-sm font-semibold text-slate-700
                 placeholder:text-slate-400 focus:outline-none" v-model.number="annualRepaymentRate"
    ...
    <button type="submit" class="w-full max-w-xs  rounded-md bg-primary py-2 text-sm font-semibold text-white">
      Save
    </button>
  </form>
</template>
...
const {
  value: annualRepaymentRate,
  setTouched: touchAnnualRepaymentRate,
